...
def _track_from_response(result, timeout, force_attr):
    """
    This is the function that actually creates the track object
    """
    def _fill_attr(attr_name):
        default = 0 if force_attr else None
        attr = audio_summary[attr_name] if audio_summary.get(attr_name) is not None else default
        if attr is not None:
            track[attr_name] = attr
    ...
    if not status == 'complete':
        ...
        _fill_attr('energy')
        _fill_attr('danceability')
        _fill_attr('speechiness')
        _fill_attr('liveness')
        _fill_attr('valence')
        _fill_attr('acousticness')

        return Track(identifier, md5, track)

def _upload(param_dict, timeout, data, force_attr):
    """
    ...
    return _track_from_response(result, timeout, force_attr)

def _profile(param_dict, timeout, force_attr):
    param_dict['format'] = 'json'
    param_dict['bucket'] = 'audio_summary'
    result = util.callm('track/profile', param_dict)
    return _track_from_response(result, timeout, force_attr)
...
def _track_from_data(audio_data, filetype, timeout, force_attr):
    param_dict = {}
    param_dict['filetype'] = filetype
    return _upload(param_dict, timeout, audio_data, force_attr)

def track_from_file(file_object, filetype, timeout=DEFAULT_ASYNC_TIMEOUT, force_upload=False, force_attr=True):
    """
    ...
    Args:
        ...
        force_attr: set True to force a value for all the audio attributes to be created.
                    If not present in The Echo Nest data, 0 will be used.
    Example:
    ...
    if not force_upload:
        try:
            ...
            return track_from_md5(md5, force_attr=force_attr)
        except util.EchoNestAPIError:
            # Fall through to do a fresh upload.
            pass

    file_object.seek(0)
    return _track_from_data(file_object.read(), filetype, timeout, force_attr)

def track_from_filename(filename, filetype = None, timeout=DEFAULT_ASYNC_TIMEOUT, force_upload=False, force_attr=True):
    """
    ...
    result = track_from_file(file_object, filetype, timeout, force_upload, force_attr)
    file_object.close()
    return result

def track_from_url(url, timeout=DEFAULT_ASYNC_TIMEOUT, force_attr=True):
    """
    ...
    return _upload(param_dict, timeout, data=None, force_attr=force_attr)

def track_from_id(identifier, timeout=DEFAULT_ASYNC_TIMEOUT, force_attr=True):
    """
    ...
    return _profile(param_dict, timeout, force_attr)

def track_from_md5(md5, timeout=DEFAULT_ASYNC_TIMEOUT, force_attr=True):
    """
